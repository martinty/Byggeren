...
uint8_t CAN_init(){
	...
	GICR    |=  (1<<INT1);		// Enable interrupt on INT1
...
void CAN_error(){
	...
	if(error & RXWAR){
		printf("CAN error: Receive Buffer 0 Overflow Flag bit");
	}else if( error & TXWAR){
		printf("CAN error: Receive Buffer 0 Overflow Flag bit");
	}
...
CAN_message CAN_receive(){
	...
	if(rxFlag){
		...
		messange.lenght =  0x0F & MCP2515_read(MPC_RXB0DLC);
...
void CAN_handler(){
	if(rxFlag){
		...
		canBuffer.lenght =  0x0F & MCP2515_read(MPC_RXB0DLC);
